...
public class FeedbackServiceImpl implements FeedbackService {
	...
	public BasicRes fillin(FillinReq req) throws Exception {
		...
		// 3.檢查同一個email是否已填寫過該張問卷
		int emailCount = feedbackDao.selectCountByEmailAndQuizId(req.getEmail(), req.getQuizId());
		if (emailCount > 0) {
		...
		try {
			for (QuesIdAnswerVo vo : req.getAnswerVoList()) {
				...
				feedbackDao.insert(req.getUserName(), req.getPhone(), req.getEmail(), req.getAge(), req.getQuizId(),
						vo.getQuesId(), answerStr, now);
			}
		} catch (Exception e) {
			throw e;
		}
