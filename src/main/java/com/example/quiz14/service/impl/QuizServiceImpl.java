...
public class QuizServiceImpl implements QuizService {
	 ...
	 * 2. 只有 cacheNames(或value) 沒有key，會把 cacheNames 是 getAll 的所有暫存資料清除</br>
	...
	public GetQuestionRes getQuestionByQuizId(int quizId) throws JsonProcessingException {
		...
		for (Question item : questionList) {
			...
			// 要把原本資料型態是List<String>的字串(String)轉回成原本的格式List<String>(參考create新增問題的mapper)
			try {
	...
	public BasicRes update(UpdateReq req) throws Exception {
		// 檢查quiz的id和QuestionVo中的quizID是否一致
	...
	public SearchRes getAll(SearchReq req) {
		...
		if (!StringUtils.hasText(quizName)) {
			// 若quizName是null或空字串或全空白字串時，設定成空字串
			quizName = "";
		}
...
//	@Scheduled(fixedRateString = "${fixed.rate}") 
	...
	// cron後面的字串有6個單位，從左到右依序是 秒 分 時 日 月 星期幾，單位間要有空格
	@Scheduled(cron = "* * * * * *")
