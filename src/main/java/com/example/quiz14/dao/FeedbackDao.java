...
public interface FeedbackDao extends JpaRepository<Feedback, FeedbackId> {
	...
	@Query(value = "insert into feedback " //
			...
			+ " values " //
			+ " (:userName, :phone, :email, :age, :quizId, :quesId, :answer, :fillinDate)", nativeQuery = true)
	public void insert(//
	...
	/**
	 * nativeQuery = false時， SQL中的語法<br>
	 * 1.select的欄位名稱會變成各個Entity class中的屬性變數名稱<br>
	 * 2.on後面的欄位名稱會是Entity class中的屬性變數名稱<br>
	 * 3.表的名稱會變成Entity class名稱<br>
	 * 4.select後面的欄位要透過new建構方法的方式來塞值，FeedbackDto中也要有對應的建構方法<br>
	 * 5.FeedbackDto要給定完整的路徑com.example.quiz14.vo.FeedbackDto
	 */
